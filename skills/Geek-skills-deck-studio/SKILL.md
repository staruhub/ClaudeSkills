...
### Step 5 页级结构
先按页面类型从 `references/layout-registry.md` **选注册版式并声明 `layout: LXX`**（不从零发明构图），再逐页填 page schema（`references/page-schema.md`）：页面类型、核心结论、视觉结构、内容模块、图像需求。
...
| 中文标点炸脚本 | 模式 B 用 PptxGenJS/JS 生成时，中文引号写成直引号导致 `SyntaxError` | JS 字符串里的中文引号一律用书名号「」或全角引号，别用会和定界符冲突的直引号 |
| 全盘细体假高级 | 把"越大越细"执行成所有字都变细，画面单薄疲软无锚点（盲评 3:0 落败的真实教训） | 极端对比 = 极细与极重**共存**：粗黑标题/数据英雄做锚点，细体只做对比层 |
| pptx 字体 fallback | PptxGenJS 直出 pptx 不嵌字体，打开端缺字体时版面崩坏 | 视觉优先场景走 HTML→Chrome 截图→贴图管线（字体由渲染机锁定）；要可编辑才用 PptxGenJS 并声明字体条件 |
| 正文半角标点 | 中文句子里混半角冒号逗号，大屏上廉价感明显 | 中文正文标点一律全角；上标与底数间距手工收紧 |
...
| `references/layout-registry.md` | Step 5 选版式时；14 个注册版式（来自三套实证样例）+ 正交拆维组织原则 |
| `references/visual-checklist.md` | 渲染后自检时；原典量化 12 条（Duarte/Tufte/M-B/Butterick/clreq）+ 实战教训 10 条 |
| `references/style-roadmap.md` | 扩风格库前必读；26 来源普查的评级表 + 反面清单 + ≤14 套终态蓝图 |
...
| `examples/constructivist-design-constitution/` | 构成主义红 9 页（评审 7.1/10 全场最高，首破 7 分线），需强母题/宣言气质的 deck 照此起步 |

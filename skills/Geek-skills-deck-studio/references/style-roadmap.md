...
**用户定制补充**（2026-08）：用户从 9 卡 picker 明确要求的 6 个风格已入库为 `huabu-stamp`（画布彩章）、`classic-desktop`（经典桌面）、`cobalt-brief`（钴蓝简报）、`coral-night`（珊瑚夜色）、`electric-grid`（电光网格）、`emerald-gazette`（翡翠公报）。这些不改变终态 ≤14 哲学（策展制 vs 堆量），但响应用户明确需求补充场景覆盖（Windows 95 怀旧、暗色路演、科技网格、杂志式公报等）。
